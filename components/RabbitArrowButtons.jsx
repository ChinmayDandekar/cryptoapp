...
const RabbitArrowButtons = ({ className }) => {
  ...
  const handleRightArrowButton = () => {
    document.getElementById("container").scrollLeft -= 275
  }
  ...
  return (
    <div className={`absolute left-[50%] translate-x-[-50%] lg:top-40 ${className}`} >
      <div className='flex flex-row relative text-cyan font-bold'>
        <div onClick={handleRightArrowButton} className='bg-light py-2 pl-2 pr-10 rounded-l-full active:bg-[lightgrey]' >
          <FaArrowLeft />
        </div>
        <div onClick={handleLeftArrowButton} className='bg-light py-2 pr-2 pl-10 rounded-r-full active:bg-[lightgrey]'>
          <FaArrowRight />
        </div>
